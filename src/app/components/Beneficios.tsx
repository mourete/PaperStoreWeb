    ...
    const services = [
      {
        ...
        description:
          "Configuración para la estructura operativa del levantamiento información a través de instrumentos diseñados y controlados por asignación, recolección, consistencia para maximizar la veracidad  de los datos.",
      ...
      <section className="py-16 bg-gray-50">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {services.map((service, index) => (
            <div
              key={index}
              className="relative rounded-lg overflow-hidden shadow-lg transform transition-transform duration-300 hover:scale-105"
            >
              <img
                src={service.image}
                alt={service.title}
                className="w-full h-48 object-contain bg-white transition-transform duration-300 transform group-hover:scale-105"
              />
              <div className="absolute inset-0 bg-gray-900 bg-opacity-70 font-sans flex flex-col justify-center items-center p-4 text-white">
